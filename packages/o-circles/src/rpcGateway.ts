...
export class RpcGateway {
  ...
  static async trigger(
    f: (web3: Web3) => Promise<any>,
    timeoutAndRotateAfterMs: number
  ) {
    for (let i = 0; i < RpcGateway.gateways.length; i++) {
      try {
        const observable = (w3: Web3) =>
          <any>new Observable<any>((subscriber) => {
            f(w3)
        ...
        if (e === "slow_provider" || e.message === "slow_provider") {
          console.warn(
            "The provider took too long to answer. Retrying with a different provider ..."
          );
    ...
    throw new Error(
      `The request could not be completed in time. Tried ${RpcGateway.gateways.length} providers.`
    );
  }
  ...
  static rotateProvider() {
    ...
    if (this._lastProviderUrl) {
      ...
      availableGateways = this.gateways.filter(
        (o) => this.gateways.length == 1 || o != this._lastProviderUrl
      );
    }

    // Choose a provider at random
    const nextProvider =
      availableGateways[Math.floor(Math.random() * availableGateways.length)];
    ...
    providerInstance.on("close", <any>((e: any) => {
      console.error(
        `The RPC gateway (${nextProvider}) closed the connection:`,
        e
      );
      this.rotateProvider();
    }));
