...
<script lang="ts">
  ...
  window.o.runProcess = async function runProcess (processDefinition:ProcessDefinition<any, any>, contextData:{[x:string]:any}, dirtyFlags:{[x:string]:boolean}|undefined) {
  ...
  <div class="flex flex-col text-base">
    <main class="z-30 flex-1 overflow-y-auto">
      <div
        ...
        class:mb-16={(!modal ||!modal.isOpen()) &&
          lastLoadedDapp &&
          lastLoadedDapp.dappId !== "homepage:1"}
        class:blur={modal && modal.isOpen()}
      >
        <Router
          routes={_routes}
