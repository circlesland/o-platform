...
import {
  ...
  FindRouteResult,
} from "../../../shared/functions/findRoutableByParams";
...
export const dappFrame = createMachine<
  DappFrameStateContext,
  DappFrameStateEvent
>(
  {
    initial: "empty",
        ...
        always: [
          {
            ...
            actions: [
              "showRoutableAsMainContent",
              send({ type: "CLOSE" }, { to: (ctx) => ctx._centerDialog }),
            ],
            target: "ready",
          },
          {
    ...
    guards: {
      ...
      routableIsMainContent: (ctx) => {
        ...
        if (ctx.routable.routable.type !== "page")
          throw new Error(
            `The "routable" is not of type "page" its a "${ctx.routable.routable.type}".`
          );
    ...
    services: {
      findAndSetRuntimeDapp: async (ctx, event) => {
        ...
        if (event.type !== "URL_CHANGED")
          throw new Error(
            `Expected a URL_CHANGED event but got ${event.type}.`
          );
    ...
    actions: {
        ...
        routable: (ctx, event) => {
          ...
          if (!findRouteResult.found && ctx.runtimeDapp.defaultRoute) {
            ...
            const defaultRoutable = findRoutableByParams(ctx.runtimeDapp, {
              ...
              "1":
                ctx.runtimeDapp.defaultRoute.length > 0
                  ? ctx.runtimeDapp.defaultRoute[0]
                  : null,
              "2":
                ctx.runtimeDapp.defaultRoute.length > 1
                  ? ctx.runtimeDapp.defaultRoute[1]
                  : null,
              "3":
                ctx.runtimeDapp.defaultRoute.length > 2
                  ? ctx.runtimeDapp.defaultRoute[2]
                  : null,
              "4":
                ctx.runtimeDapp.defaultRoute.length > 3
                  ? ctx.runtimeDapp.defaultRoute[3]
                  : null,
              "5":
                ctx.runtimeDapp.defaultRoute.length > 4
                  ? ctx.runtimeDapp.defaultRoute[4]
                  : null,
              "6":
                ctx.runtimeDapp.defaultRoute.length > 5
                  ? ctx.runtimeDapp.defaultRoute[5]
                  : null,
            });
      ...
      setMainContent: assign({
        layout: (ctx, event) => {
          ...
          if (event.type != "CONTENT_CHANGED")
            throw new Error(
              `Expected a CONTENT_CHANGED event but got ${event.type}.`
            );
          if (event.position !== "main")
            throw new Error(
              `Expected a CONTENT_CHANGED event with position "main" but got ${event.position}.`
            );
      ...
      setModalIsOpen: assign({
        layout: (ctx, event) => {
          ...
          if (event.type != "OPENED" && event.type != "CLOSED")
            throw new Error(
              `Expected one of OPENED, CLOSED events but got ${event.type}.`
            );
          if (event.position !== "center")
            throw new Error(
              `Expected one of OPENED, CLOSED events with position "center" but got ${event.position}.`
            );
      ...
      setModalContent: assign({
        layout: (ctx, event) => {
          ...
          if (event.position !== "center")
            throw new Error(
              `Expected a CONTENT_CHANGED event with position "center" but got ${event.position}.`
            );
      ...
      showRoutableAsModalContent: send(
        (ctx) => {
