<script lang="ts">
  ...
  import {CrcHubTransfer, ProfileEvent} from "../../../../shared/api/data/types";
  ...
  function getValues() : {
    ...
    const messageTag = hubTransfer.tags.find(o => o.typeId === "o-banking:transfer:message:1");
    if (messageTag) {
      text = messageTag.value;
    }

    if (event.direction == "in") {
      title = `${event.contact_address_profile.firstName} sent you ${displayCirclesAmount(
        hubTransfer.flow,
        null,
        true,
