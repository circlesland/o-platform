<script lang="ts">
  ...
  import {ContactDirection, ContactPoint, ContactPointSource} from "../../../shared/api/data/types";
  ...
  onMount(() => {
    ...
    const trustMetadata:ContactPointSource = contact.metadata.find(p => p.name === "CrcTrust");
    let trustIn = 0;
    let trustOut = 0;
...
<div on:click="{() => loadDetailPage(safeAddress)}">
  <ItemCard
    params="{{ edgeless: false, imageProfile: contact.contactAddress_Profile, title: displayName, subTitle: message, truncateMain: true }}">
