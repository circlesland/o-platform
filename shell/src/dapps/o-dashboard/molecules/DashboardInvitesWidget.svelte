...
<section class="relative p-4 mb-4 bg-white border cardborder ">
  <div class="absolute top-0 left-0 w-full text-center">
    {#if _stats.goals.nextGoal > 0}
      <progress
        class="w-full h-10 min-h-12 rounded-t-xl progress progress-primary"
    ...
    <div class="absolute grid w-full grid-cols-3 px-2 text-white top-3">
      {#if _stats.goals.nextGoal > 0}
        <div class="inline-flex justify-start flex-none text-sm text-left">
          <div class="pr-1">{_stats.profilesCount}</div>
          <Label key="dapps.o-dashboard.molecules.dashboardInvitesWidget.citizens" />
        </div>
        ...
        <div class="-ml-2 text-sm text-right whitespace-nowrap">
          +{_stats.goals.nextGoal - _stats.profilesCount > 0 ? _stats.goals.nextGoal - _stats.profilesCount : 0}
          <Label key="dapps.o-dashboard.molecules.dashboardInvitesWidget.tillNextParty" />
        </div>
      {/if}
    </div>
  </div>
    ...
    <div class="flex flex-row justify-around mt-10">
      <div class="flex flex-col self-center space-y-2 justify-items-center">
        ...
        <div class="text-sm text-dark-lightest">
          <Label key="dapps.o-dashboard.molecules.dashboardInvitesWidget.myLeaderrank" />
        </div>
      </div>
      <div class="flex flex-col self-center space-y-2 justify-items-center">
        ...
        <div class="text-sm text-dark-lightest">
          <Label key="dapps.o-dashboard.molecules.dashboardInvitesWidget.myInvites" />
        </div>
      </div>
    </div>
  {/if}
