            ...
            {#each balances.reverse() as token}
                <section class="flex items-start pb-2 bg-white shadow-md rounded-xl">
                    <div class="flex flex-col w-full ">
                        <div class="relative flex flex-row items-center content-start p-2 space-x-4 text-base font-medium text-left bg-light-lighter">
                            <div class="inline-flex">
                                <!--<UserImage profile="{token.token_owner_profile}" size="{10}" gradientRing="{true}"/>-->
                            </div>
                            <div>
                                {token.token_name} (#{token.token_no})
                            </div>
                        </div>
                        <header class="cursor-pointer rounded-t-xl headerImageContainer"
                                on:click="{() => {}}">
                            <div class="relative rounded-t-xl image-wrapper">
                                <img src="{token.token_url ? token.token_url : '/images/market/circles-no-image.jpg'}"
                                     alt=""
                                     class="rounded-t-xl"/>
                        ...
                        <div class="relative flex flex-row items-center content-start p-2 space-x-4 text-base font-medium text-left bg-light-lighter">
                            ...
                            <div>
                                {token.token_owner_profile.displayName.length > 24 ? token.token_owner_profile.displayName.substring(0, 24) + '..' : token.token_owner_profile.displayName}
                            </div>
                        </div>
                    </div>
                </section>
