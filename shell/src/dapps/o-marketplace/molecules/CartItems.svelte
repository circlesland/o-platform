...
+each(`groupedItems as {item}`)
  div.flex.justify-between.items-center.mb-6.pb-6.border-b
    div.flex.items-center.w-full
      ...
      div.flex.flex-col.ml-2.space-y-2.w-full.items-start
        ...
        div.flex.justify-end.items-center.w-full
          div.flex-grow.text-sm.text-dark-lightest 1 {item.item.unitTag ? item.item.unitTag.value : "item"}
            
          div.pr-8.flex  
            div.span.font-semibold.cursor-pointer(on:click!="{() =>  removeOneItem(item.item.id)}") -
