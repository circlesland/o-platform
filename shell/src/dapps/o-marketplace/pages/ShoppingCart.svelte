<script lang="ts">
  ...
  async function checkMaxTransferableAmount(sumsBySeller: { [sellerAddress: string]: number }) {
    const maxTransferableAmounts: { sellerAddress: string, maxFlow: BN }[] = await Promise.all(
      Object.keys(sumsBySeller)
        .map(async sellerAddress => {
            const flow = await ApiClient.query<TransitivePath, QueryDirectPathArgs>(
              DirectPathDocument,
              {
                from: $me.circlesAddress,
                to: sellerAddress,
                amount: RpcGateway.get().utils.toWei((new Currency().convertTimeCirclesToCircles(sumsBySeller[sellerAddress], null) * 10).toString(), "ether").toString()
              });
              ...
              <div class="flex items-center justify-center mt-6">
                <div class="flex flex-row w-full space-x-4">
                  <div class="flex-grow">
                    {#if !checked}
                      <button
                              class="h-auto btn-block btn btn-disabled"
                      >{$_(
