...
$: {
  pictureUrl = symbol;
  varietyDetail = variety == 0 || variety == 1 ? title : variety ? variety + " different " + title : "";
}
</script>

<div on:click="{() => push(`#/banking/assets/${symbol}`)}" class="cursor-pointer">
  <ItemCard
    params="{{
      edgeless: false,
      ...
      endTextSmall: variety == 0 || variety == 1 ? title : `${Number.parseFloat(balance / 10).toFixed(2)} €`,
      mobileTextCutoff: 18,
    }}" />
</div>
