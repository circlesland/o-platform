  ...
  {#each balances as token}
    {#if token && token.token_balance > 0}
      <div on:click={() => push(`#/friends/${token.token_owner_address}`)}>
        <ItemCard params="{{
            edgeless: false,
            imageUrl: !token.token_owner_profile || !token.token_owner_profile.avatarUrl
              ? AvataarGenerator.generate(token.token_owner_address)
              : token.token_owner_profile.avatarUrl,
            title: token.token_owner_profile
              ...
              : token.tokenOwner,
            subTitle: token.token_owner_address,
            truncateMain: true,
            shadowMedium: true
        }}">
